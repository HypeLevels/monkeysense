...
local ui_is_menu_open, ui_get, ui_set, ui_update, ui_new_color_picker, ui_new_string, ui_reference, ui_set_visible,
ui_new_listbox, ui_new_button, ui_new_checkbox, ui_new_label, ui_new_combobox, ui_new_multiselect, ui_new_slider,
ui_new_hotkey, ui_set_callback, ui_new_textbox = ui.is_menu_open, ui.get, ui.set, ui.update, ui.new_color_picker,
    ui.new_string, ui.reference, ui.set_visible, ui.new_listbox, ui.new_button, ui.new_checkbox, ui.new_label,
    ui.new_combobox, ui.new_multiselect, ui.new_slider, ui.new_hotkey, ui.set_callback, ui.new_textbox
local globals_realtime, globals_curtime, globals_tickcount, globals_maxplayers = globals.realtime, globals.curtime,
    globals.tickcount, globals.maxplayers
...
local entity_get_local_player, entity_get_player_weapon, entity_get_classname, entity_get_prop,
entity_get_player_resource, entity_get_origin, entity_get_players, entity_get_esp_data, entity_get_game_rules,
entity_is_enemy, entity_is_alive, entity_get_player_name = entity.get_local_player, entity.get_player_weapon,
    entity.get_classname,
    entity.get_prop, entity.get_player_resource, entity.get_origin, entity.get_players, entity.get_esp_data,
    entity.get_game_rules, entity.is_enemy, entity.is_alive, entity.get_player_name
local client_color_log, client_timestamp, error_log, client_reload_active_scripts, client_set_event_callback,
client_latency, client_current_threat, client_userid_to_entindex, client_screen_size = client.color_log,
    client.timestamp, client.error_log, client.reload_active_scripts, client.set_event_callback, client.latency,
    client.current_threat, client.userid_to_entindex, client.screen_size
...
local select, setmetatable, toticks, require, tonumber, tostring, ipairs, pairs, type, pcall, writefile, assert, print,
printf = select, setmetatable, toticks, require, tonumber, tostring, ipairs, pairs, type, pcall, writefile, assert,
    print, printf
--#endregion
...
local hitgroup_names = { 'generic', 'head', 'chest', 'stomach', 'left arm', 'right arm', 'left leg', 'right leg', 'neck',
    '?', 'gear' }
local dimensions = { client.screen_size() }

-- Menu color hex codes. '\aRRGGBBAA'
local WHITE, LIGHTGRAY, GRAY, GREEN, YELLOW, LIGHTRED, RED = "\aFFFFFFE1", "\aAFAFAFE1", "\a646464E1", "\aAFFFAFE1",
    "\aFFFF96E1", "\aFFAFAFE1", "\aFF8080E1"
...
local references = {
    ...
    body_aim = ui_reference("RAGE", "Aimbot", "Force Body Aim")
}
...
local menu = {
    ...
    new = ui_new_button("AA", "Anti-aimbot angles", GREEN .. "New", function()
    end),
    edit = ui_new_button("AA", "Anti-aimbot angles", "Edit", function()
    end),
    edit_inactive = ui_new_button("AA", "Anti-aimbot angles", GRAY .. "Edit", function()
    end),
    toggle = ui_new_button("AA", "Anti-aimbot angles", "Toggle", function()
    end),
    toggle_inactive = ui_new_button("AA", "Anti-aimbot angles", GRAY .. "Toggle", function()
    end),
    move_up = ui_new_button("AA", "Anti-aimbot angles", "Move up", function()
    end),
    move_up_inactive = ui_new_button("AA", "Anti-aimbot angles", GRAY .. "Move up", function()
    end),
    move_down = ui_new_button("AA", "Anti-aimbot angles", "Move down", function()
    end),
    move_down_inactive = ui_new_button("AA", "Anti-aimbot angles", GRAY .. "Move down", function()
    end),
    delete = ui_new_button("AA", "Anti-aimbot angles", RED .. "Delete", function()
    end),
    ...
    cond_toggle = ui_new_button("AA", "Anti-aimbot angles", "Toggle", function()
    end),
    finish = ui_new_button("AA", "Anti-aimbot angles", "Finish editing", function()
    end),
    back = ui_new_button("AA", "Anti-aimbot angles", "Back", function()
    end),
    descriptions = {},
    ...
    next = ui_new_button("AA", "Anti-aimbot angles", "Next", function()
    end),
    back2_saved = ui_new_button("AA", "Anti-aimbot angles", "Back", function()
    end),
    back2_unsaved = ui_new_button("AA", "Anti-aimbot angles", "Back" .. RED .. " [unsaved]", function()
    end),
...
local function set_references_visibility(b)
    set_table_visibility(b, references.pitch, references.pitch_val, references.yaw_base, references.yaw,
        references.yaw_val, references.jitter, references.jitter_val, references.body, references.body_val,
        references.freestand_body, references.roll, references.edge_yaw, references.freestanding,
        references.freestanding_key)
end
...
local function update_browser()
    ...
    for i, v in ipairs(blocks) do
        display[#display + 1] = v.enabled and string_format("%s[%i] %s%s", LIGHTGRAY, num, WHITE, v.name) or
            string_format("%s[  ] %s%s", LIGHTGRAY, GRAY, v.name)
...
local function update_cond_browser()
    ...
    for _, v in ipairs(custom_conditions) do
        display[#display + 1] = string_format("%s[c] %s%s", YELLOW,
            includes(current_block.conditions, v) and WHITE or GRAY, v)
    end
...
local function update_cond_description(condition)
    ...
    local description = condition .. ": " ..
        (DESCRIPTIONS[condition] or custom_descriptions[condition] or "None provided.")
...
local function update_visibility(s)
    ...
    set_table_visibility(screen == 0, menu.browser, menu.new, menu.import_config, menu.export_config)
    ...
    set_table_visibility(screen == 2, menu.cond_type, menu.cond_browser, menu.cond_toggle, menu.finish,
        unpack(menu.descriptions))
...
local function get_total_enemies()
    ...
    for e = 1, globals_maxplayers() do
        if entity_get_prop(entity_get_player_resource(), "m_bConnected", e) and entity_is_enemy(e) and
            entity_is_alive(e) then
            count = count + 1
        end
    end
...
local function add_setting(name, target_ref, block_ref, default)
    ...
    custom_settings[name] = { target_ref, block_ref, def }
...
local function get_condition_list()
    ...
    for _, v in ipairs(CONDITIONS) do
        all_conditions[#all_conditions + 1] = v
    end
    for _, v in ipairs(custom_conditions) do
        all_conditions[#all_conditions + 1] = v
    end
...
local OutlineGlow = function(x, y, w, h, radius, r, g, b, a)
    renderer.rectangle(x + 2, y + radius + 8, 1, h - 8 * 2 - radius * 2, r, g, b, a)
    renderer.rectangle(x + w - 3, y + radius + 8, 1, h - 8 * 2 - radius * 2, r, g, b, a)
    renderer.rectangle(x + radius + 8, y + 2, w - 8 * 2 - radius * 2, 1, r, g, b, a)
    renderer.rectangle(x + radius + 8, y + h - 3, w - 8 * 2 - radius * 2, 1, r, g, b, a)
    renderer.circle_outline(x + radius + 8, y + radius + 8, r, g, b, a, radius + 6, 180, 0.25, 1)
    renderer.circle_outline(x + w - radius - 8, y + radius + 8, r, g, b, a, radius + 6, 270, 0.25, 1)
    renderer.circle_outline(x + radius + 8, y + h - radius - 8, r, g, b, a, radius + 6, 90, 0.25, 1)
    renderer.circle_outline(x + w - radius - 8, y + h - radius - 8, r, g, b, a, radius + 6, 0, 0.25, 1)
end

function renderer.outlined_rounded_rectangle(x, y, w, h, r, g, b, a, radius, thickness, glow)
    y = y + radius
    local data_circle = { { x + radius, y, 180 }, { x + w - radius, y, 270 }, { x + radius, y + h - radius * 2, 90 },
    ...
    for _, data in next, data_circle2 do
        renderer.circle(data[1], data[2], 20, 20, 20, 255, radius, data[3], 0.25)
    end

    for _, data in next, data2 do
        renderer.rectangle(data[1], data[2], data[3], data[4], 20, 20, 20, 255)
    end
    ...
    local n = a / 255 * 40;
    local y = y - 8
    renderer.rectangle(x + radius, y, w - radius * 2, 1, r, g, b, a * 255)
    renderer.circle_outline(x + radius, y + radius, r, g, b, a * 255, radius, 180, 0.25, 1)
    renderer.circle_outline(x + w - radius, y + radius, r, g, b, a * 255, radius, 270, 0.25, 1)
    renderer.gradient(x, y + radius, 1, h - radius * 2, r, g, b, a * 255, r, g, b, n, false)
    renderer.gradient(x + w - 1, y + radius, 1, h - radius * 2, r, g, b, a * 255, r, g, b, n, false)
    ...
    for radius = 4, glow * 25 do
        local radius = radius / 2;
        OutlineGlow(x - radius, y - radius, w + radius * 2, h + radius * 2, radius, r, g, b, glow * 25 - radius * 2)
    end
end

local renderNotification = function()
...
local addNotification = function(text, duration, pngtype, color)
    table_insert(notifications, {
        text = text,
        endTime = globals_realtime() + duration,
        tween = 0,
        r = color[1],
        g = color[2],
        b = color[3],
...
local paint = function()
    local alpha = math.clamp(math.abs(math.sin(globals.curtime() * 2)) * 255, 100, 255)
    local scoped = entity_get_prop(entity_get_local_player(), 'm_bIsScoped') == 1
    xoffset = easing.linear(globals.frametime(), xoffset, (scoped and 45 or 0) - xoffset, .1)
...
--#region fire events
local aim_fire = function(ctx)
    stored_shot = {
        damage = ctx.damage,
        hitbox = hitgroup_names[ctx.hitgroup + 1],
        lagcomp = ctx.teleported,
        backtrack = globals_tickcount() - ctx.tick
    }
end

local aim_miss = function(ctx)
    local string = string.format("missed %s's %s due to %s [dmg: %s, bt: %s, lc: %s]",
        string.lower(entity_get_player_name(ctx.target)), stored_shot.hitbox, ctx.reason, stored_shot.damage,
        stored_shot.backtrack, stored_shot.lagcomp)
    addNotification(string, 4, monkeyred, { 255, 0, 0 })
    color_print({ 200, 200, 200, "[monkeysense] " }, { 255, 0, 0, "Missed " }, { 200, 200, 200,
        string_format("%s's %s due to ",
            entity_get_player_name(ctx.target), stored_shot.hitbox) }, { 255, 0, 0, ctx.reason }, { 200, 200, 200,
        string.format(
            " [dmg: %s, bt: %s, lc: %s]", stored_shot.damage, stored_shot.backtrack, stored_shot.lagcomp) })
end

local aim_hit = function(ctx)
    local string = string.format("hit %s for %s [%s] in the %s [%s] [hc: %s, bt: %s, lc: %s]",
        string.lower(entity_get_player_name(ctx.target)), ctx.damage, stored_shot.damage,
        hitgroup_names[ctx.hitgroup + 1] or '?', stored_shot.hitbox, math.floor(ctx.hit_chance) .. "%",
        stored_shot.backtrack, stored_shot.lagcomp)
    addNotification(string, 4, monkeygreen, { 0, 255, 0 })
    color_print({ 200, 200, 200, "[monkeysense] " }, { 0, 200, 0, "Hit " }, { 200, 200, 200,
            string_format("%s for %s [%s] in the ",
                entity_get_player_name(ctx.target), ctx.damage, stored_shot.damage) },
        { 0, 200, 0, string.format("%s [%s] ", hitgroup_names[ctx.hitgroup + 1] or '?', stored_shot.hitbox) },
        { 200, 200, 200,
            string.format("[hc: %s, bt: %s, lc: %s]", math.floor(ctx.hit_chance) .. "%", stored_shot.backtrack,
                stored_shot.lagcomp) })
end

local da_miss = function(ctx)
    local string = string.format("missed %s's %s [%s] due to dormancy [hc: %s]", entity_get_player_name(ctx.userid),
        ctx.aim_hitbox, ctx.aim_point, math.floor(ctx.accuracy * 100) .. "%")
    addNotification(string, 4, monkeyred, { 255, 0, 0 })
    color_print({ 200, 200, 200, "[monkeysense] " }, { 200, 0, 0, "Missed " }, { 200, 200, 200,
            string_format("%s's %s [%s] due to ",
                entity_get_player_name(ctx.userid), ctx.aim_hitbox, ctx.aim_point) }, { 200, 0, 0, "dormancy " },
        { 200, 200, 200, string.format("[hc: %s]", math.floor(ctx.accuracy * 100) .. "%") })
end

local da_hit = function(ctx)
    local string = string.format("hit %s's %s [%s] for %s damage (%s left) [hc: %s]",
        entity_get_player_name(ctx.userid), ctx.aim_hitbox, ctx.aim_point, ctx.dmg_health, ctx.health,
        math.floor(ctx.accuracy * 100) .. "%")
    addNotification(string, 4, monkeygreen, { 0, 255, 0 })
    color_print({ 200, 200, 200, "[monkeysense] " }, { 0, 200, 0, "Hit " },
        { 200, 200, 200, string_format("%s's ", entity_get_player_name(ctx.userid)) },
        { 0, 200, 0, string.format("%s [%s] ", hitgroup_names[ctx.hitgroup + 1], ctx.aim_hitbox) }, { 200, 200, 200,
            string.format(
                "for %s damage (%s left) [hc: %s]", ctx.dmg_health, ctx.health, math.floor(ctx.accuracy * 100) .. "%") })
end
--#endregion
...
local function on_init()
    ...
    client_set_event_callback("paint_ui", renderNotification)
    ...
    ui_set_callback(menu.new, function()
        new_block = true;
        current_block = Block();
        update_values();
        update_visibility(1)
    end)
    ui_set_callback(menu.edit, function()
        new_block = false;
        current_block = blocks[ui_get(menu.browser) + 1];
        update_values();
        update_visibility(1)
    end)
    ui_set_callback(menu.toggle, function()
        blocks[ui_get(menu.browser) + 1].enabled = not blocks[ui_get(menu.browser) + 1].enabled;
        update_browser()
    end)
    ui_set_callback(menu.delete, function()
        table_remove(blocks, ui_get(menu.browser) + 1);
        update_browser();
        if #blocks > 0 then
            ui_set(menu.browser, ui_get(menu.browser) - 1)
        end
        update_visibility(0)
    end)
    ui_set_callback(menu.next, function()
        update_visibility(2)
    end)
    ui_set_callback(menu.back, function()
        update_visibility(1)
    end)
    ui_set_callback(menu.pitch, function()
        if screen == 1 then
            update_visibility(1)
        end
    end)
    ui_set_callback(menu.yaw, function()
        if screen == 1 then
            update_visibility(1)
        end
    end)
    ui_set_callback(menu.jitter, function()
        if screen == 1 then
            update_visibility(1)
        end
    end)
    ui_set_callback(menu.body, function()
        if screen == 1 then
            update_visibility(1)
        end
    end)
    ui_set_callback(menu.finish, function()
        current_block:update();
        if new_block then
    ...
    ui_set_callback(menu.back2_saved, function()
        current_block:update();
        current_block = nil;
        update_visibility(0)
    end)
    ui_set_callback(menu.back2_unsaved, function()
        current_block = nil;
        update_visibility(0)
    end)
    ui_set_callback(menu.import_config, function()
        if json_parse(clipboard.get()) then
            ui.set(menu.config, clipboard.get())
            load_config()
            addNotification("Successfuly imported config", 4, monkeyblue, { 0, 0, 255 })
        end
    end)
    ui_set_callback(menu.export_config, function()
        clipboard.set(save_config())
        addNotification("Successfuly exported config", 4, monkeyblue, { 0, 0, 255 })
    end)
    ...
    builder.save_config = function()
        return save_config()
    end
    builder.load_config = function()
        return load_config()
    end
